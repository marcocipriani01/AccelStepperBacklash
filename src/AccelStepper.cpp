...
boolean AccelStepper::runSpeed()
{
    ...
    if (time - _lastStepTime >= _stepInterval)
    {
        ...
        long stepUnit = ((_direction == DIRECTION_CW) ? 1 : (-1));
        if (backlashing)
        {
            _currentBacklash += stepUnit;
            step(_currentPos + _currentBacklash);
            return true;
        }
        _currentPos += stepUnit;
        step(_currentPos + _currentBacklash);
        return true;
    }
    else
...
void AccelStepper::step1(long step)
{
    ...
    setOutputPins((_direction == DIRECTION_CCW) ? 0b10 : 0b00); // Set direction first else get rogue pulses
    setOutputPins((_direction == DIRECTION_CCW) ? 0b11 : 0b01); // step HIGH
    // Caution 200ns setup time
    // Delay the minimum allowed pulse width
    delayMicroseconds(_minPulseWidth);
    setOutputPins((_direction == DIRECTION_CCW) ? 0b10 : 0b00); // step LOW
}
...
boolean AccelStepper::runSpeedToPosition()
{
    if (_targetPos == _currentPos)
        return false;
    applyBacklashCompensation((_targetPos > _currentPos) ? DIRECTION_CW : DIRECTION_CCW);
    return runSpeed();
}
